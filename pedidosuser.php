...
$sql="SELECT id, data, nome, de, ate, das, ashoras, observacoes, obsdiretor FROM pedidos WHERE id_utilizador='$_SESSION[userid]' AND estado='0'";
...
<div class="tab-pane fade" id="nav-aceites" role="tabpanel" aria-labelledby="aceites">
  ...
  $sql="SELECT id, data, nome, de, ate, das, ashoras, observacoes, obsdiretor FROM pedidos WHERE id_utilizador='$_SESSION[userid]' AND estado='1'";
  $result= mysqli_query($conn, $sql);
  echo "<table class='table table-hover'>";
  if(mysqli_num_rows($result) >0){
    while($row= mysqli_fetch_assoc($result)){
...
<div class="tab-pane fade" id="nav-recusados" role="tabpanel" aria-labelledby="recusados">
  ...
  $sql="SELECT id, data, nome, de, ate, das, ashoras, observacoes, obsdiretor FROM pedidos WHERE id_utilizador='$_SESSION[userid]' AND estado='2'";
  $result= mysqli_query($conn, $sql);
  echo "<table class='table table-hover'>";
  if(mysqli_num_rows($result) >0){
    while($row= mysqli_fetch_assoc($result)){
